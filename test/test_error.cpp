...
TEST_F(TestErrorPointToPoint, JacobianUpdate) {
  ...
  expected_result <<  -0.142513353825048, 0.026363566665563, 0.004115131755677, -0.075215663389197, 0.014601072079337,
                  -0.706267106105253;
  ...
  expected_result2 << 0.000031103263367, -0.050288499760193, 0.002192279140143, -0.015684760231580, -0.015728590131416,
                   -0.000120155047649 ;
...
TEST_F(TestErrorPointToPoint, TranlationPartOfConstrainedJacobianUpdate) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    pc2_d->push_back(pcl::PointXYZ(i + 1, i + 1, i + 1));
    Jnc.row(i * 3) << -1, 0, 0;
    Jnc.row(i * 3 + 1) << 0, -1, 0;
    Jnc.row(i * 3 + 2) << 0, 0, -1;
...

  {
    ...
    ASSERT_TRUE(Jnc.isApprox(JJ)) << "Error, wrong jacobian, expected " << Jnc << " actual: " << JJ;
  }
